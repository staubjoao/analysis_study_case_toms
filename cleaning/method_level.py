...
class MethodLevelDecompositionConstructor:
    ...
    def migrate_class_decompositions(self, application, tool, partition_count, variant):
        ...
        tomicroservices_variants = ["tomicroservices_p1", "tomicroservices_p2", "tomicroservices_p3"]
        method_level_tools = ["tomicroservices", "tomicroservices_p1", "tomicroservices_p2", "tomicroservices_p3", "cargo", "mosaic", "ground_truth"]
        if tool in method_level_tools or tool in tomicroservices_variants:
                ...
                for partition_count in self.manifest.get_partition_count_options(
                    ...
                    for variant in self.manifest.get_decomposition_variants(tool):
                        ...
                        if self.manifest.get_granularity(tool) == "class":
                            granularities.append("class_level")
                        for granularity in granularities:
                            if "method" in granularity and tool not in ["cargo", "tomicroservices", "tomicroservices_p1", "tomicroservices_p2", "tomicroservices_p3", "mosaic", "ground_truth"]:
